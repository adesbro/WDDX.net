...
namespace Mueller.Wddx
{
    ...
    public class WddxDeserializer
    {
        ...
        /// <summary>
        ///     Deserializes a string containing a WDDX packet into an instance of a specified type. 
        ///     Does not perform validation on the packet.
        /// </summary>
        /// <typeparam name="T">The type of the instance to deserialize into.</typeparam>
        /// <param name="WddxPacket">The WDDX packet to be deserialized.</param>
        public T Deserialize<T>(string WddxPacket)
        {
            return this.Deserialize<T>(WddxPacket, false);
        }
        ...
        ///		The <c>validate</c> parameter is set to <c>true</c> and invalid WDDX is encountered.
        ///	</exception>
        public T Deserialize<T>(string WddxPacket, bool validate)
        {
            T retVal;
            ...
            using (StringReader stream = new StringReader(WddxPacket))
            {
                ...
                reader.WhitespaceHandling = WhitespaceHandling.None;
                reader.Namespaces = false;
                retVal = this.Deserialize<T>(reader, validate);
                reader.Close();
            }
        ...
        /// <summary>
        ///		Deserialize an <see cref="System.Xml.XmlTextReader"/> pointing to a WDDX packet into an 
        ///		instance of a specified type. Does not perform validation on the packet.
        /// </summary>
        /// <typeparam name="T">The type of the instance to deserialize into.</typeparam>
        /// <param name="input">The pre-initialized <see cref="System.Xml.XmlTextReader"/> pointing to the WDDX to be parsed.</param>
        public T Deserialize<T>(XmlTextReader input)
        {
            return this.Deserialize<T>(input, false);
        }
        ...
        ///		The <c>validate</c> parameter is set to <c>true</c> and invalid WDDX is encountered.
        ///	</exception>
        public T Deserialize<T>(XmlTextReader input, bool validate)
        {
            if (validate)
            {
                WddxValidator validator = new WddxValidator();
                return validator.Deserialize<T>(input);
            }
        ...
        private static void ReadXmlText(XmlReader input, Action<XmlReader> nodeAction)
        {
            int a = 0;
